...
<div class="mt-16">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        ...
        <a href="{{route('posts.show',$post)}}" class="relative p-6 bg-white from-gray-700/50 via-transparent rounded-lg shadow-2xl shadow-gray-500/20 motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500">
            <div class="absolute left-0 top-0 bg-indigo-600 text-white rounded-tl-lg py-1 px-2 text-sm">{{$post->category->name}}</div>
            <div class="flex">
            ...
            <div class="text-right">
                <p class="text-xs text-gr-400">Fecha de publicacion: {{ $carbon::parse($post->updated_at)->diffForHumans() }} </p>
                    <p class="text-xs">Hora: {{ $post->updated_at}}  </p>
